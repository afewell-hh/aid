...
- **Fixture generator:** `kernel/tools/gen-fixtures-mbt.mjs` (whose sole output was the deleted `fixtures_gen.mbt`) is deleted, and `gen-fixtures.sh` no longer emits any MoonBit source, so nothing resurrects the retired kernel path. The toy fixtures under `tests/fixtures/{valid,invalid}` themselves are left in place (their `plan.yaml` is still read by `cmd/aid` tests) and belong to the separate D20 toy-fixture retirement, not this WIT/topology-IR cluster.
