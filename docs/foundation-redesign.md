...
- A **`transceiver_cage`** is a pluggable slot that *can* be populated with a compatible transceiver — e.g. the CX-7's one QSFP112 @ 400G and the BF3's two QSFP112 @ 200G (`real-server-bom.csv:10-11`; `README.md:20-21`). This is the model object the GUI binds to for "select a transceiver to populate this cage," and it is what lets the BOM reducer add the per-cage transceiver as a required subcomponent (it is *not* a line in the flat CSV, `README.md:17-19`).
  > **Implementation gate (devb re-review): keep catalog *capability* separate from plan *binding*.** In the **catalog**, a cage declares only *constraints* — `cage_type`, `max_speed_gbps`, `requires_transceiver`, `allowed_transceivers[]`. The **selected** transceiver is an **instance/configuration binding made on the plan/server-class**, not stored on the catalog item — *unless the SKU truly ships with a fixed/captive optic*, which is then a `fixed_interface` (or a non-pluggable cage with a baked selection). This keeps catalog items reusable across plans and lets one catalog cage be populated differently per design.
